    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-xl font-semibold text-brand-black mb-4 flex items-center">
                <svg class="w-6 h-6 text-brand-elite mr-2" fill="currentColor" viewBox="0 0 20 20">
                    <path
            ...
            <div class="space-y-3">
                @forelse($admins as $admin)
                    <div class="flex items-center justify-between p-4 bg-brand-light rounded-lg">
                        ...
                        @else
                            <span class="bg-brand-elite text-white px-4 py-2 rounded text-sm">
                                Super Admin
                            </span>
                        @endif
                    </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <input type="text" wire:model.live="search" placeholder="Search users..."
                class="w-full px-4 py-2 mb-4 border border-brand-gray rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-elite">
                ...
                @forelse($standardUsers as $user)
                    <div class="flex items-center justify-between p-4 bg-brand-light rounded-lg">
                        ...
                        <button wire:click="promoteToAdmin({{ $user->id }})"
                            class="bg-brand-elite text-white px-4 py-2 rounded text-sm hover:bg-opacity-90 transition">
                            Promote
                        </button>
                    </div>
                @empty
